"""Evaluate the trained EAGLE3 draft vs the baseline Llama-3.2-1B on MT-Bench."""
import argparse
import csv
import time
...
def run(method: str) -> None:
    ...
    with out_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for i, item in enumerate(prompts):
            ...
            out = llm.generate([item["prompt"]], sp, use_tqdm=False)
...
def main() -> None:
    ...
    ap.add_argument("--method", choices=["baseline", "eagle3"], required=True)
    args = ap.parse_args()
    run(args.method)
